...
export default function ElementEditor({ elementData, onUpdate, onSave }) {
    ...
    const handleStyleChange = (styleType, value) => {
        try {
            ...
            setJsonErrors(prev => {
                ...
                delete newErrors[styleType];
                return newErrors;
            });

        } catch (error) {
           console.error('Invalid JSON for style:', error);
            // Set pesan error untuk styleType yang relevan
            setJsonErrors(prev => ({
                ...prev,
                [styleType]: 'Format JSON tidak valid.'
            }));
        }
    };
    ...
    const renderStyleEditor = (styleType, placeholder) => {
        ...
        const error = jsonErrors[styleType];

        const [jsonString, setJsonString] = useState(JSON.stringify(styleData, null, 2));
        
        useEffect(() => {
            // Hanya update jika tidak ada error, untuk mencegah menimpa input user yang salah
            if (!error) {
                setJsonString(JSON.stringify(styleData, null, 2));
            }
        }, [styleData]);

        const handleLocalChange = (e) => {
            const newString = e.target.value;
            setJsonString(newString); // Update state lokal secara langsung
            handleStyleChange(styleType, newString); // Validasi dan update state utama
        };
        return (
            <div>
                ...
                {/* JSON Editor */}
                <textarea
                    value={jsonString} // Gunakan state lokal
                    onChange={handleLocalChange} // Gunakan handler lokal
                    className={`w-full p-2 border rounded font-mono text-xs mb-2 ${error ? 'border-red-500 ring-red-500' : ''}`}
                    rows="8"
                    placeholder={placeholder}
                />
                {error && <p className="text-xs text-red-600 -mt-1 mb-2">{error}</p>}
